...
function filterPreviousUploads(uploadListPath, objects) {
  ...
  var uploadedFileList = fs.readFileSync(uploadListPath, {encoding: 'utf8'});
...
function getAllFileObjects() {
  ...
  }).then(function(objectsWithFiles) {
    var objectsWithValidFileNames = _.filter(objectsWithFiles, function(anObject) {
      return (!anObject.fileName) ? false : true;
    });
    return Promise.resolve(objectsWithValidFileNames);
  });
}
